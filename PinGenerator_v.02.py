...
def write_list_to_files(number_list, min_length, filename_base="pins"):
    current_file = 1
    current_file_size = 0
    ...
    with open(f"{filename_base}{current_file}.txt", "w") as f:
        for lists in pin_lists:      
            for item in lists:
                ...
                if current_file_size > size_limit:   #Splits the output files by size
                    ...
                    current_file += 1
                    current_file_size = 0
                    f = open(f"{filename_base}{current_file}.txt", "w")
                f.write(line)
